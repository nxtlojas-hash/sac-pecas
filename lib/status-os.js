...
// Decide qual lista de etapas a tela vai usar: a que o backend mandou, ou a
// copia local (fallback).
//
// Por que existe: ate 28/07 o os-lista.js tinha uma COPIA hardcoded das 5
// ETAPAS_OS do google-apps-script.js. Duas listas do mesmo enum em arquivos
// diferentes so ficam iguais por disciplina — e justo agora elas estao MUDANDO
// ('Pronto p/ retirar' -> 'Finalizado'). Basta um deploy em que so um dos lados
// sobe pro seletor oferecer um rotulo que atualizarStatusOS recusa
// ('Status invalido. Use: ...'). Com o backend mandando `etapas` na resposta de
// listar_os, existe UMA fonte da verdade.
//
// O FALLBACK NAO E OPCIONAL: a v45 esta no ar agora e nao manda `etapas`
// (medido 28/07: listar_os devolve {ok,oses,total,exibidos} e nada mais). Sem
// fallback o seletor de status ficaria VAZIO ate a v46 subir — a tela pararia
// de mover OS. Mesma regra da respostaHistoricoUtilizavel (lib/busca-texto.js):
// campo novo so vale se vier utilizavel, e o silencio do backend velho nunca
// pode quebrar a tela.
//
// Rejeita e cai no fallback quando: nao veio, nao e array, ou nao sobrou nenhum
// rotulo aproveitavel (tudo em branco / nada que seja string). Uma lista parcial
// e aproveitada pelo que tem de bom — perder uma etapa e menos grave que perder
// as cinco, e a etapa que faltar continua aparecendo no seletor do cartao pelo
// caminho do "fora do padrao" (montarOpcoesStatus).
//
// Valores CRUS (sem trim), pelo mesmo motivo de statusBrutoDoItem: eles viram
// value de <option> e depois querystring, e o backend compara por igualdade
// exata. Funcao PURA: nao le DOM, nao faz fetch.
function etapasDaResposta(resposta, fallback) {
  var padrao = Object.prototype.toString.call(fallback) === '[object Array]' ? fallback : [];
  if (!resposta || Object.prototype.toString.call(resposta.etapas) !== '[object Array]') {
    return padrao;
  }

  var limpas = [];
  for (var i = 0; i < resposta.etapas.length; i++) {
    var et = resposta.etapas[i];
    // So string: um numero ou um objeto no enum seria bug do backend, e virar
    // "[object Object]" numa <option> e pior que ignorar.
    if (typeof et !== 'string' || !et.trim()) continue;
    if (limpas.indexOf(et) !== -1) continue;
    limpas.push(et);
  }

  return limpas.length ? limpas : padrao;
}
...
if (typeof module !== 'undefined') module.exports = {
  ...
  etapasDaResposta: etapasDaResposta
};
